...
faq:
  - q: "Should every custom card have reminder text?"
    a: "No. Follow rarity conventions: commons and uncommons carry reminder text for any keyword beyond the evergreen basics; rares and mythics usually drop it. If the text box is crowded, reminder text is the first thing to cut."
  - q: "How long is too long for a text box?"
    a: "Real cards rarely exceed four lines of rules text plus flavor. If your design needs more, it usually wants to lose an ability — the best custom cards do one or two things crisply. PipGlyph's preview shrinks text to fit, but if you're relying on that, take the hint."
  - q: "Where do I check how a real keyword is templated?"
    a: "Search any real card with the keyword on Scryfall and copy its phrasing. PipGlyph's Scryfall import can pull a real card's oracle text straight into the editor as a reference starting point."
---
...
## Let the AI do the templating

PipGlyph's built-in AI can write the rules text for you: give the card a name and a theme like "a goblin captain who rallies attackers", tick **Rules text** in the Generate with AI dialog, and it returns properly templated oracle text using real keyword vocabulary — fitted to whatever else you've already filled in. It's the fastest way to learn the grammar: describe, compare, absorb. Sign up free and try it in the [card creator](/create); every account gets 5 AI credits to start.
