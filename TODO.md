...
- [ ] **Email delivery for admin → user messages.** Today a new message only creates an in-app notification (`lib/messages/notify.ts`) and the "Messages" nav entry; users who don't log back in never see it. When the owner is ready: send a transactional email through Resend (`RESEND_API_KEY` + a verified `ADMIN_ALERT_FROM` sender) from `sendAdminMessageAction` / `replyToThreadAction`, with a "Reply on PipGlyph" link to `/messages/[threadId]`, a per-user "email me when an admin messages me" preference (default on), and no email for the user's own replies. Keep the send `after()` the DB write so a Resend outage never fails the message.
