...
//   - /sets          → static/ISR, never reads searchParams, default view
//   - /sets/browse   → dynamic, parses searchParams (reached via the
//                      proxy.ts rewrite when q/page params are present —
//                      the visitor's URL stays /sets?…)
//
...
export function parseSetsParams(
...
export function SetsView({ page, search }: SetsViewParams) {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      {/* The Suspense boundary contains SetsSearch's useSearchParams() CSR
  ...
  return (
    <>
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {sets.map((set) => (
          <PublicSetTile key={set.id} set={set} isAuthed={false} />
        ))}
      </div>
      {hasPrev || hasMore ? (
      ...
      >
        <div className="relative aspect-video w-full overflow-hidden bg-elevated">
          {set.cover_url ? (
            <Image
              src={set.cover_url}
              alt={`${set.title} — custom card set cover`}
              fill
